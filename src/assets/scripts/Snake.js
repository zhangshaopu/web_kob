...
export class Snake extends AcGameObject{
    constructor(info , gamemap){
        ...
        this.cells = [new Cell(info.r,info.c)]; // cells[0]存放蛇头，存放蛇的身体 
        this.next_cell = null;
    ...
    check_tail_increasing() {  // 检测当前回合，蛇的长度是否增加
        if (this.step <= 10) return true; // 前10回和身体边长
    ...
    update_move() { 
        ...
        if (distance < this.eps) {  // 走到目标点了
            ...
            if (!this.check_tail_increasing()) { // 蛇不变长，蛇尾走到下个目的地
                const k = this.cells.length;
    ...
    render() {
        const L = this.gamemap.L; // 每个单元格长度
        const ctx = this.gamemap.ctx; // 画布的引用
        ...
        for (const cell of this.cells) { // of遍历值
            ctx.beginPath();
        ...
        for (let i = 1; i < this.cells.length; i ++ ) {
            ...
            if (Math.abs(a.x - b.x) < this.eps && Math.abs(a.y - b.y) < this.eps) // 如果两个点重合就不画了
                continue;
